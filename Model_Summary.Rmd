...
p2<- var_agg %>% 
  ...
  geom_hline(yintercept = cod_acl())+
  geom_point(size = .5)+
  geom_jitter(width= 0.2, size = .5)+
 # geom_point(aes(data = catch_agg, aes(x = run_number, y = Value_cod, color = under_acl_cod)))+
  #ggplot2::scale_color_stepsn( limits = c(0,100), n.breaks = 10, colors = my_palette, name = "% Under Cod ACL") #+
  geom_point(data = catch_agg, aes(x = run_number, y = Value_cod),color = "blue")+
  ylab("Metric tons")+
  xlab(element_blank())+
  ggtitle("Cod Total Catch Mortality")
...
p2<- var_agg %>% 
 ...
 # geom_point(aes(data = catch_agg, aes(x = run_number, y = Value_cod, color = under_acl_cod)))+
  #ggplot2::scale_color_stepsn( limits = c(0,100), n.breaks = 10, colors = my_palette, name = "% Under Cod ACL") #+
  geom_point(data = catch_agg, aes(x = run_number, y = Value_had),color = "blue")+
  ylab("Metric tons")+
  xlab(element_blank())+
  ggtitle("Haddock Total Catch Mortality")
...
median_Value<- var_agg %>% 
  dplyr::group_by(Category, run_number) %>% 
  dplyr::summarise(median_val = median(Value))

p2<- var_agg %>% 
  ggplot(aes(x = run_number, y = Value))+
  geom_boxplot()+
  geom_hline(yintercept = cod_acl())+
  geom_point(data = median_Value,aes( x = run_number, y = median_val))+
  ylab("Metric tons")+
  xlab(element_blank())+
  ggtitle("Cod Total Catch Mortality")
...
cv_agg<- df2 %>% 
          ...
          dplyr::summarise(Value = (sum(as.numeric(Value))*-1)/1000000) 

median_Value<- cv_agg %>% 
  dplyr::group_by( run_number) %>% 
  dplyr::summarise(median_angler_satisfaction = median(Value))
...
p3<- cv_agg %>% 
  ...
  geom_point(data = median_Value,aes( x = run_number, y = median_angler_satisfaction))+
  ylab("million $")+
  xlab(element_blank())+
  ggtitle("Change in angler satisfaction in US dollars")
