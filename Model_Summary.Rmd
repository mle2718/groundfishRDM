...
df2<- df %>% dplyr::mutate(run_number = as.character(rep(fnames2$run_name, each = 6030))) %>% 
  dplyr::select(!V1) %>% 
  dplyr::filter(!run_number %in% c("MJP9", "MJP10"))
...
OK_runs<- catch_agg %>% 
  dplyr::filter(under_acl_cod >= 50 & under_acl_had >= 50)

OK_runs <- unique(OK_runs$run_number)
...
dat<- df2 %>%
      dplyr::filter(catch_disposition %in% c("keep", "Discmortality"),
                    ...
                    run_number %in% OK_runs) %>%
      dplyr::group_by(run_number, Category,draw_out) %>%
      dplyr::summarise(Value = sum(as.numeric(Value))) %>%
      dplyr::mutate(Value = Value * lb_to_mt()) %>% 
  dplyr::mutate(under_acl = dplyr::case_when(Category == "cod" & Value <= cod_acl() ~ 1, TRUE ~ 0)) %>% 
  dplyr::mutate(shaded = dplyr::case_when(run_number %in% c("AC3", "ML1", "JS11", "JS14", "JS17", "JS12",
                                                            "AC2", "JS9", "JS6", "MJP7")~ "Shade", TRUE ~ "NoShade"), 
                shaded = dplyr::case_when(run_number %in% c("SQ", "Opt1", "KB2", "MJP8")~ "Recommended", TRUE ~ shaded))
...
cv_plot<-ggplot(cv, aes(x = run_number, y = diff, color = shaded)) +
  geom_point(alpha = .4,  size = 2) + 
 scale_color_manual(values=c("#56B4E9", "blue", "grey"))+ # Jittered points
  geom_point(data = dat2, aes(x = run_number, y = median),
  ...
  ylim(ylim_l, ylim_u)  +
  scale_y_continuous(breaks = seq(round(-2.5, 2), round(10,2), by = 0.5))+
    theme(legend.position="none")
