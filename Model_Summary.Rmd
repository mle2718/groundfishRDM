...
month_file_names <- list.files(path=here::here("output/"),pattern = "monthly_output_*.Rds",full.names = T)
...
adjusted_removals<-adjusted_removals %>%
   dplyr::filter(metric %in% c("removals_weight"))
...
adjusted_removals<-adjusted_removals %>%
      dplyr::mutate(Value = as.numeric(Value) * scaling_factor* lb_to_mt())
...
adjusted_removals<-adjusted_removals %>%
      dplyr::mutate(under_acl = dplyr::case_when(species == "cod" & adjusted_mortality_mt <= cod_acl() ~ 1, TRUE ~ 0),
                    under_acl = dplyr::case_when(species == "had" & adjusted_mortality_mt <= had_acl() ~ 1, TRUE ~ under_acl)) %>%
...
## Mortality
total_mortality<- function(model){

 file <- list.files(path = here::here("output"), pattern = paste0("output_", model, "_"),full.names = TRUE ) |>
    tibble(file = _) |>
