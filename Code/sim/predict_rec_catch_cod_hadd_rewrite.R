...
#   calib_catch_draws_{draw}.fst OR proj_catch_draws_{draw}.fst
#   proj_year_calendar_adjustments.fst
...
read_projection_common_inputs_cod_hadd <- function(season_draw,
                                                   mode_draw,
                                                   draws) {
  ...
  if (nrow(calendar_adjustments)) {
    if (!"expansion_factor" %in% names(calendar_adjustments)) calendar_adjustments[, expansion_factor := 1]
  }
...
simulate_species_project_cod_hadd <- function(catch_dt,
  ...
  fish_dt <- pos_dt[rep(seq_len(.N), pmax(catch_n, 0L))]
...
project_one_cod_hadd <- function(s, md, dr, common_inputs) {
  ...
  base_outcomes[, date_parsed := parse_date_any(date_parsed)]
  ...
  n_choice_occasions[, date_parsed := parse_date_any(date_parsed)]
...
predictions_list <- vector("list", length(season_draw) * length(mode_draw) * length(draws))
k <- 1L
...
system.time({
  for (s in season_draw) {
    for (md in mode_draw) {
      for (dr in draws) {
        message("Projection: season=", s, ", mode=", md, ", draw=", dr)
        out <- project_one_cod_hadd(s = s, md = md, dr = dr, common_inputs = common_inputs)
        if (!is.null(out) && nrow(out)) {
          predictions_list[[k]] <- out
          k <- k + 1L
        }
      }
    }
...
prediction_draws <- data.table::rbindlist(predictions_list, use.names = TRUE, fill = TRUE)

# Build true all-mode totals after both modes have been processed.
prediction_all_modes <- prediction_draws[
  mode != "all modes",
  .(value = sum(value, na.rm = TRUE)),
  by = .(season, iteration, metric)
][, mode := "all modes"]

prediction_draws <- data.table::rbindlist(
  list(prediction_draws, prediction_all_modes),
  use.names = TRUE,
  fill = TRUE
)
...
calib <- copy(common_inputs$calib)
