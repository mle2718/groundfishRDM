...
simulate_cod <- function(md, s) {
  ...
  floor_subl_cod_harv<-min(directed_trips_sub$cod_min_y2_alt)-3*2.54

  if (min(directed_trips_sub$cod_min_y2_alt)==100){
  ...
  if (cod_catch_check != 0) {
    ...
    cod_catch_data[, posskeep := as.integer(fitted_length >= cod_min_y2_alt)]
    cod_catch_data[, csum_keep := ave(posskeep, tripid, date, mode, catch_draw, FUN = cumsum)]
    cod_catch_data[, keep_adj := as.integer(posskeep == 1 & csum_keep <= cod_bag_y2_alt)]
...
simulate_hadd <- function(md, s) {
  data.table::setDTthreads(1)
    ...
    floor_subl_hadd_harv<-min(directed_trips_sub$hadd_min_y2_alt)-3*2.54

    if (min(directed_trips_sub$hadd_min_y2_alt)==100){
    ...
    if (hadd_catch_check != 0) {
      ...
      # Identify keepable fish
      hadd_catch_data[, posskeep := as.integer(fitted_length >= hadd_min_y2_alt)]
      hadd_catch_data[, csum_keep := ave(posskeep, tripid, date, mode, catch_draw, FUN = cumsum)]
      hadd_catch_data[, keep_adj := as.integer(posskeep == 1 & csum_keep <= hadd_bag_y2_alt)]
