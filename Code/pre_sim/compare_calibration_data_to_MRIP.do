...
ds mode month disp species domain, not has(type numeric)
local var = r(varlist)
...
ds mode disp species domain, not has(type numeric)
local var = r(varlist)
foreach v of local var{
	format `v' %14.0gc
}
...
ds mode season disp species domain, not has(type numeric)
local var = r(varlist)
foreach v of local var{
	format `v' %14.0gc
}
