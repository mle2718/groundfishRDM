...
keep if inlist(st,23, 33, 25) //ensure only relevant states
...
***TEST to compare whether catch-at-length distributions in cod open season is different than CaL in entire year 
* current data is (year==2025 & inlist(wave, 1, 2, 3)) | (year==2024 & inlist(wave,  4, 5, 6))"

* FY 2025 regs (year==2025 wave 3, 4, 5, 6; 2026 wave 1, 2 ) 
	* GoM cod: sept & oct 2025, 1 fish 23"
	* GoM haddock: May 2025 - February 2026, April 2026, 15 fish 18"

* FY 2024 regs (year==2024 wave 3, 4, 5, 6; 2025 wave 1, 2 ) 
	* GoM cod: sept & oct 2024, 1 fish 23"
	* GoM haddock: May 2024 - February 2025, April 2025, 15 fish 18"


*create a variable indicating if the observation came from a month where the season was open or closed
gen cod_open = 1 if inlist(month1, 9, 10)
mvencode cod_open, mv(0)

gen season="op" if cod_open==1
...
* for cod, use unweighted b2 length data, weighted a+b1 length data
* for haddock, use weighted b2 length data, weighted a+b1 length data
...
drop  nfish_ab1 nfish_b2 sum_ab1 sum_b2
tempfile props
save `props', replace 


*********************
* Now estimate total keep/release by species month and multiply proportions calculated above
cd $input_data_cd
...
drop if disp=="catch"
reshape wide b, i(species season) j(disp) string
merge 1:m season species using `props'
drop if species=="z"


gen nfish_ab1=prop_ab1*bkeep
gen nfish_b2=prop_b2*brel
mvencode nfish_ab1  nfish_b2, mv(0) override
gen nfish_catch=nfish_ab1+nfish_b2
order  species season l_cm_bin nfish_catch
collapse (sum) nfish_catch  , by( species season l_cm_bin)
drop if species==""
...
twoway(scatter observed_prob l_cm_bin if species=="cod" & season=="open", connect(direct) lcol(red)    lpat(solid) msymbol(i) ) ///
			(scatter observed_prob l_cm_bin if species=="cod" & season=="closed", connect(direct) lcol(blue)    lpat(solid) msymbol(i) ) 
			
			///
twoway(scatter observed_prob l_cm_bin if species=="hadd" & season=="open", connect(direct) lcol(green)   lwidth(medthick)  lpat(solid) msymbol(i) ) ///
			(scatter observed_prob l_cm_bin if species=="hadd" & season=="closed", connect(direct) lcol(orange)   lwidth(medthick)  lpat(solid) msymbol(i) ) 
...
egen sum_observed_prob=sum(observed_prob), by(domain)
replace observed_prob=observed_prob/sum_observed_prob

egen sum_fitted_prob=sum(fitted_prob), by(domain)
drop sum_observed_prob sum_fitted_prob

*graphs the fitted observed/fitted probabilities
...
/*
*statisitical and visual tests to see if distributions of cod catch-at-length differ between open v. closed season. 
keep if species=="cod"
expand nfish 
sort season length
twoway (kdensity length if season=="open") ///
       (kdensity length if season=="closed"), ///
       legend(order(1 "open" 2 "closed")) ///
       title("Cod catch-at-length distributions")
	   
sort season length
cumul length if season=="open", gen(cdf0)
cumul length if season=="closed", gen(cdf1)

twoway (line cdf0 length) (line cdf1 length), legend(order(1 "Group 0" 2 "Group 1"))
ksmirnov length, by(season)

*confirmed length distributions differ based on *point estimates* of MRIP data 
*/


save "$input_data_cd/.dta", replace  //This file has the fitted catch-at-length probabilities in the baseline year
